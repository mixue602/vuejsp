                    ...
                    <label  @click.stop="onAn(index,val,val.taf)"><img :src="val.brdimg"  alt=""><i :class="{i:val.taf}"></i></label>
                     <transition name="slide-fade">
                        <ul class="elUl clearfix" v-show="menuobj[index].taf">
                            <li class="ellipsis waves"  @click.stop="onshow(index,index2,$event)"  v-for="(val2,index2) in goodsobj" :class="{'on':val2.taf}">{{ val2.spunm }}
                            </li>
                        </ul>
                    </transition>
                </li>
    ...
    methods:{
        ...
        onshow (index,index2,$event) {
            ...
            setTimeout(function(){
                ...
                that.goodsobj[index2].taf = true;
                that.onIndex = index;
                that.goodsRender = that.goodsobj;
                that.$emit("addpramas",that.parentIndex,that.goodsRender,that.onIndex,2); 
            }, 200)
        ...
        addPramas() {
            if(this.parentIndex !==null){
                this.$emit("addpramas",this.parentIndex,this.goodsRender,2); 
            }      
        }
