    ...
    <div>
        <div class="subinfo">
            <ul class="list-labelvalue">
                ...
                <li id="GmTime" style="border-bottom: 1px solid #f0f0f0;">
                    <label>购买时间</label>
            ...
            <ul class="list-labelvalue">
                <li id="serviceMes" >
                    <label>留言（选填）</label>
                    <textarea placeholder="请输入其他要求或故障描述(100字内)" maxlength="100" v-model="mas" @blur="setMas"></textarea>
                </li>
            </ul>
        </div>
...
import timer from './selecttime';
    ...
    export default {
        ...
        methods: {
            // 处理URL参数到页面
            _setParmasToPage() {
                let parmas = this.savequickrepair.submit || {},
                day,
                date;
                if (parmas.timeStamp) {
            ...
            _getAddr() {
                let that = this,
                    ...
                    surl,
                    tsup,
                    savequickrepair =this.savequickrepair || {},
                    submit = savequickrepair.submit;
                ...
                //     this.$emit("setmas",this.mas)
                // };
                if(this.savequickrepair.submit.mas!=null){
                    this.mas =  this.savequickrepair.submit.mas;
                    this.$emit("setmas",this.mas)
                };
                   // debugger:start
                    // var res = {
            ...
            chooseAddr() {
                ...
                ss.setItem('savequickrepair',JSON.stringify(this.savequickrepair));
                    temp.tsup= tsup;
                    //跳转回来的页面
                    temp.page = this.page;
