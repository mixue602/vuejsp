<template>
	<div id="ordersDetail" class="wapper">
		...
		<div class="container" style="width:10rem;left:initial;">
            <!-- 配送状态 -->
            <div class="delivery-state t14"  @click.stop="fnOrderLv(detailResult.dodt)">
                <div class="staff-info">
                    <div class="staff-info-profile" v-if="detailResult.servant.headIcon"  @click.stop="fnServant(detailResult.servant.id,detailResult.dodt,detailResult.serviceOrgId,detailResult.gdls[0].gdid)"><img :src="detailResult.servant.headIcon"/></div>
                    <div class="staff-info-desc">
                        <p class="staff-info-name" v-if="detailResult.servant.name">服务人员：{{ detailResult.servant.name }}</p>
                        <p class="staff-info-name"><span class="star-five t12" v-for="n in detailResult.servant.starRated">★</span></p>
                        <!-- <p class="staff-info-num">已服务1人</p> -->
                    </div>
                </div>
                <div class="state-right" v-if="odtype == 10485761">{{orderstate.sw[detailResult.odst]}}</div>
                <div class="state-right" v-else-if="odtype == 8388608">{{orderstate.dzsg[detailResult.odst]}}</div>
                <div class="state-right" v-else>{{orderstate[detailResult.odst]}}</div>
            </div>
            <!-- 订单状态 -->
            <div class="order-info" v-if="detailResult.odstDesc" @click.stop="fnOrderLv(detailResult.dodt)">
                <i class="bg" :class=""></i>
                <div class="desc green t13 bdr-top":class="">{{detailResult.odstDesc}}</div>
                <div class="color999 t11 border pd10">{{ detailResult.otut | gvTime }}</div>
                <i class="i i-gt"></i>
            </div>
			<!-- 配送地址 -->
		    <ul class="list-line">
		    	<li v-show="detailResult.cname&&detailResult.faddr">
		    		<div class="area-info t13">
                        ...
                        <div class="color999 pd10 bdr-bottom">{{detailResult.faddr}}</div>
                    </div>
		    	</li>
		    </ul>
		    <!-- 商品列表 -->
		    <div class="order-shopBar t13">国美管家</div>
    		<!--     <div class="orderInfo">
                    <div class="clearfix order-info-d1" v-for="(v2,i2) in detailResult.gdls" @click="fnOrderLv(detailResult.oid,detailResult.shippingGroupId,detailResult.dodt,detailResult.sotp,detailResult.servant)">
                        <div class="float_l orderImg">
                            <img :src="v2.gdiul" alt="">
                        </div>
                ...
                <!-- 充值订单信息 -->
                <div class="orderInfo" v-if="odtype == 10485760">
                    <div class="clearfix order-info-d1" v-for="(v,i) in detailResult.gdls">
                        <div class="float_l orderImg">
                            <img src="//gfs13.gomein.net.cn/T1wIL_BCAT1RCvBVdK.png" alt="">
                        </div>
                        <div class="float_r order-info-yb">
                            <p class="order-info-yb-p1">{{ orderTypeName[odtype] }}</p>
                            <p class="clearfix order-info-yb-p2">
                                <span class="float_l s1">手机号码:&nbsp;{{ v.mob }}</span>
                            </p>
                            <p class="clearfix order-info-yb-p3">充值面额:&nbsp;{{ v.gdpr/100 }}元</p>
                        </div>
                    </div>
                </div>
                <!-- 充值订单信息 -->
                <!-- 维修订单信息 -->
                <div class="orderInfo" v-else-if="odtype == 4194305">
                    <div class="clearfix order-info-d1" v-for="(v,i) in detailResult.gdls">
                        <div class="float_l orderImg">
                            <img :src="v.gdiul" alt="">
                        </div>
                        <div class="float_r order-info-yb">
                            <p class="order-info-yb-p1">{{ orderTypeName[odtype] }}</p>
                            <p class="clearfix order-info-yb-p2">
                                <span class="float_l s1">{{ v.title }}</span>
                            </p>
                            <p class="clearfix order-info-yb-p3">{{ v.desc }}</p>
                        </div>
                    </div>
                </div>
                <!-- 维修订单信息 -->
                ...
                <div class="orderInfo" v-else>
                    <div class="clearfix order-info-d1" v-for="(v,i) in detailResult.gdls">
                        <div class="float_l orderImg">
                            <img :src="v.gdiul" alt="">
                            <img src="//gfs10.gomein.net.cn/T1FZDgB5xT1RCvBVdK.png" alt="" v-if="odtype == 6291456">
                        </div>
                        <div class="float_r order-info-yb">
                            <p class="order-info-yb-p1">{{ orderTypeName[odtype] }}</p>
                            <p class="clearfix order-info-yb-p2">
                                <span class="float_l s1" v-if="odtype == 10485761">{{ v.gdnm }}</span>
                                <span class="float_l" v-else>{{ v.title }}</span>
                                <span class="float_r s2">x{{ v.gdnu }}</span>
                            </p>
                            <p class="order-info-yb-p4">￥{{ v.gdpr/100 }}</p>
                        </div>
                    </div>
                </div>
            <!-- 延保清洗订单信息 -->
		    ...
		    <div class="order-bg">
			    <ul class="custom-service t14 bdr-bottom">
			    	<li><a :href="'tel:'+detailResult.serviceOrgPhone"><span class="icon seller-tel"></span>商家电话</a></li>
			    	<!-- <li><a href="tel:4008113333"><span class="icon service-tel"></span>在线客服</a></li> -->
			    </ul>
			</div>
		    <ul class="list-labelvalue t13">
		        <li class="bdr-bottom" v-if="detailResult.gdls[0].desc && odtype == 4194305 || odtype == 4194307 ">
		        	<div>
		       		    <label>故障</label>
		                <span class="value" id="otn" @click="showCopy">{{ detailResult.gdls[0].desc }}</span>
		            </div>
		        </li>
                <li class="bdr-bottom" v-if="detailResult.msg">
                    <div>
                        <label>备注</label>
                        <span class="value" id="otn" @click="showCopy">{{ detailResult.msg }}</span>
                    </div>
                </li>
                <li class="bdr-bottom" v-if="detailResult.imgUrls">
                    <div>
                        <label>上传图片</label>
                        <div class="imgUrls clearfix">
                            <img :src="v" class="float_l" alt="" v-for="(v,i) in detailResult.imgUrls" @click="picShow(i)">
                        </div>
                    </div>
                </li>
                <li class="bdr-bottom" v-if="oidF">
                    <div>
                        <label>订单编号</label>
                        <span class="value" id="otn" @click="showCopy">{{ oidF }}</span>

                    </div>
                    <!-- 复制浮层 -->
                    <dl class="copy-layer dn">
                        <a @click="copy" href="javascript:;" v-bind:data-clipboard-text="detailResult.ordNm" class="copyBtn t15" id="copyBtn">复制</a>
		        ...
		        <li class="bdr-bottom" v-show="odtype != 10485761 && odtype != 10485760 && odtype != 6291456">
		        	<div>
		                <label>服务方式</label>
		                <span class="value" id="sdodt">上门配送</span>
		            </div>
		        </li>
		        <li class="bdr-bottom" v-if="detailResult.ots">
		        	<div>
		                <label>预约日期</label>
		                <span class="value" id="sdodt">{{ detailResult.ots | gvTime2 }}</span>
		            </div>
		        </li>
		    </ul>
    	    <ul class="list-labelvalue t13 mt10 mbottom140">
                <li>
                    <div>
                        <label>商品总价</label>
                        <span class="value" id="gtam" v-if="odtype == 10485761">{{ detailResult.gdls[0].gdnu }}件<i class="t12">¥</i> {{ detailResult.gtam | numInt }}<i class="t12">{{ detailResult.gtam | numFloat }}</i></span>
                        <span class="value" v-else><i class="t12">¥</i> {{ detailResult.gtam | numInt }}<i class="t12">{{ detailResult.gtam | numFloat }}</i></span>
                    </div>
                    ...
                    <div>
                        <label>实付款</label>
                        <span class="value" v-if="detailResult.odst == 1 || detailResult.odst == 33">¥ <i>{{ detailResult.ramp | numInt }}</i>0</span>
                        <span class="value" v-else>¥ <i>{{ detailResult.ramp | numInt }}</i>{{ detailResult.ramp | numFloat }}</span>
                    </div>
                </li>
            </ul>
		</div>
		<div class="footer t13">
            <span class="btn btn-linear mr disabled" v-show="detailResult.odst == 4 || detailResult.odst == 355 || detailResult.odst == 388 || detailResult.odst == 34 " @click.stop="fnComment(detailResult.oid,detailResult.sotp,detailResult.gdls[0].gdid,detailResult.shippingGroupId)">评价</span>
            <span class="btn btn-linear mr" v-show="detailResult.odst == 1 || detailResult.odst == 33 && odtype != 4194307" @click="fnPay(detailResult.dodt)">支付</span>
            <span class="btn btn-linear mr" v-if="odtype == 6291456 && detailResult.gdls[0].late" @click="fnTk(detailResult.ordNm)">退款</span>
            <span class="btn btn-linear mr" v-else-if="detailResult.odst == 4 && tkTime < 1" @click="fnTk(detailResult.ordNm)">退款</span>
            <span class="btn btn-linear mr" v-show="detailResult.odst != 1 && detailResult.odst != 33 && odtype == 6291456" @click="fnInvoicedetail(detailResult.ordNm,detailResult.sotp)">查看发票</span>
        </div>
	</div>
</template>
<script  type="text/ecmascript-6">
	...
	export default {
            ...
            return {
	            ...
	            orderstate: {
	            	...
	            	zt:{
						...
						"REMOVED": "已取消",
						"FAILED": "配送失败",
						"PENDING_SHIPMENT": "等待配送",
						"NO_PENDING_ACTION":"已妥投",
						"PENDING_MERCHANT_ACTION": "待客服处理",
						"DELIVERY": "已出库",
						"REJECT_BACK": "拒收入大库",
						"REJECT": "拒收",
						"PAUSED": "配送单已暂停",
						...
						"PENDING_ORDER_CONFIRM": "等待用户收货确认",
        ...
        created() {
	        ...
	        this.getOrderDetail(this.hrefParma.otn, this.hrefParma.dodt, this.hrefParma.odtype, this.hrefParma.isJkxOtn);
            // 保存订单id
            if (this.hrefParma.isJkxOtn == 1) {
                this.oidF = this.hrefParma.otn;
            }else{
                this.oidGJ = this.hrefParma.otn;
            };
	        // 初始化预约时间日期
            for (var i = 0; i < 7; i++) {
        ...
        methods: {
            ...
            fnTk(oid) {
                    ...
                    okFn: function() {
                        util.api({
                            surl:root.GOODS_API_PATH +'refund?otn='+oid,
                            type: 'get',
                            // 去除iscroll两次单击bug
                            async: false,
			...
			addEvent:function (ind) {
				...
				if (this.detailResult.imgUrls === 1)　{
					return false;
				}
				TouchSlide({
					slideCell:"#slideBox1",
            ...
            // 管家服务人员评价
            fnServant(epnb,dodt,serviceOrgId,gdId) {
                var _this = this;
                if (!epnb) return;
            ...
            fnOrderLv(dodt) {
                var _this = this;
                // 跳管家轨迹
                if (_this.odtype == 10485760) return;
            ...
            // 售后服务列表TF
        	fnOrderSale() {
        		this.orderSaleTF = !this.orderSaleTF;
        	},
        	// 选择预约安装时间
        	fnOrderTimeZz() {
        		var _this = this;
             ...
             * 获取订单详情
             */
            getOrderDetail: function(otn, dodt, odtype, isJkxOtn) {
                var _this = this;
                // 请求
                util.api({
                    // surl: '/resources/json/ordersdetail.json',
                    ...
                    success: function(res) {
                        ...
                        switch (rpco) {
                            case 200:
                                ...
                                // 商家电话
                                if(!_this.detailResult.serviceOrgPhone) {
                                    _this.detailResult.serviceOrgPhone = 4008113333;
                                };
                                if (body.compt) {
                                    var ttime = (body.gdls[0].compt)/1000,
                                        nowtime = new Date().getTime()/1000,
                                        hours = (nowtime - ttime)/3600;
                                    _this.tkTime = hours;
                                };
                                // 存在服务人员信息， 调评价接口
                                if (_this.detailResult.servant) {
                                    _this.fnCxservant(_this.detailResult.servant.id,_this.detailResult.dodt,_this.detailResult.serviceOrgId);
                                }else{
                                ...
                                // 保存订单id
                                if (_this.hrefParma.isJkxOtn == 1) {
                                    _this.oidGJ = body.ordNm;
                                }else{
                                    _this.oidF = body.ordNm;
                                };
                                break;
                                // 没有找到对应数据
                            case 404:
