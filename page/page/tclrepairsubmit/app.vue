            ...
            <div class="subinfo" v-show="!time.timeShow">
                <ul class="list-labelvalue">
                    ...
                    <li id="serviceAddr" class="bdr-bottom" @click="chooseAddr">
                        ...
                        <span class="name" v-show="resAddr && !resAddr2">{{resTree.cname}}&nbsp;&nbsp;&nbsp;&nbsp;{{resTree.cphone}}</span>
                        <span class="addr-detail" v-show="resAddr && !resAddr2">{{resTree.addr}}</span>
...
export default {
	...
	created() {
		...
		let ordid=util.getHrefParma().serial;
        this.submitObj.thirdCode =assid;
        this.submitObj.serial =ordid;
		this.getmessage(assid,ordid);
	},
	methods:{
		// 获取资产信息
		getmessage(assid,ordid){
			var that=this;
			util.api({
	        	surl: './asc/asset?goodsId='+assid+'&serial='+ordid,
	        	type:'get',
	        	success(res) {
	        		...
	        		switch (res.rpco){
	        		    ...
	        		    case 200:
                            ...
                            that.moren.attrnm=body.title;
                             // 提交参数
                            that.submitObj.phone = body.phone;
                            that.submitObj.goodsId = body.id;
                            ...
                            //电话号码中间四位显示成*
                            that.resTree.cphone = (body.phone)?(body.phone).replace(/(\d{3})\d{4}(\d{4})/,'$1****$2'): '';
                            ...
                            if(body.phone){
                             ...
                             if(that.hrefparma.tsup!=undefined){
                                    that.submitObj.addId = that.hrefparma.tsup;
                              }


                            //
	        		        /*that.hrefParma.catename=body.cateTitle;
	        		        that.hrefParma.brdnm=body.brandTitle;
	        		        that.hrefParma.attrnm=body.name;
