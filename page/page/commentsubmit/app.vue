            ...
            <div class="goods-show oh">
                <div class="goods-show-header bd-none">
                    ...
                    <span class="xingji" score="">
                        <span v-for="(val,index) in starts" :class="{'cur': val.cur}" @click="setStar(index,val)"></span>
                    </span>
                    <span class="xingji-text">{{starLeval}}</span>
                </div>
                <div class="mt10 comment-txt pr">
                    <textarea v-model="submitObj.commentMas"  maxlength="300" name="" id="" placeholder='主人，请写下对本次服务的感受吧，您的评价对于我们完善服务很重要哦~'></textarea> <span class="txt-tip">300字以内</span>      
                </div>
            </div>
        </div>
        <footer class="waves on"  @click="commentSumit($event)">提交</footer>
    </div>
</template>
...
export default {
    ...
    methods:{
        // 得到即可修订单号
        setOtn(otn){
            this.submitObj.otn = otn;
        },
        // 星级评分
        setStar(index,val) {
            for(var i=0;i<this.starts.length;i++){
                if(i<=index){
                    this.starts[i].cur = true;
                }else{
                    this.starts[i].cur = false;
                }
            }
            switch(index) {
                case 0:
        ...
        commentSumit(event) {
            ...
            let data = {
                "grade": this.submitObj.grade,
                "comment": this.submitObj.commentMas,
                "otn": this.submitObj.otn,
                "odtype": this.hrefParma.odtype,
                "photos": []
                }
            util.api({
                surl:root.ORDER_API_PATH + 'serviceComment',
                type:'post',
                data:data,
...
#commentSubmit .comment-txt{
    position: relative;
    padding: 0.266667rem 0.266667rem 0.8rem;
...
/* textarea::-webkit-input-placeholder:after{
  display:block;
  content:"主人，请写下对本次服务的感受吧~ \A 您的评价对于我们完善服务很重要哦~";
};
