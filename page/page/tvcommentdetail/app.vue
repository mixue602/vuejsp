        ...
        <div class="wapper">
            ...
            <div class="goods-show oh">
                <div class="goods-show-header bd-none">
                    <span>综合评分</span>
                    <span class="xingji">
                        <span class="cur" v-for='item in comprehensive.goodcom'></span><span v-for='item in comprehensive.badcom'></span>
                    </span>
                    <span class="xingji-text">{{comprehensive.comtext}}</span>
                </div>
              <!--   <div>
                    <div class="goods-show-header bd-none">
                        <span>服务态度</span>
                        <span class="xingji" score="">
                            <span class="cur" v-for='item in attitude.goodcom'></span><span v-for='item in attitude.badcom'></span>
                        </span>
                        <span class="xingji-text">{{attitude.comtext}}</span>
                    </div>
                    <div class="goods-show-header bd-none">
                        <span>专业程度</span>
                        <span class="xingji" score="">
                            <span class="cur" v-for='item in major.goodcom'></span><span v-for='item in major.badcom'></span>
                        </span>
                        <span class="xingji-text">{{major.comtext}}</span>
                    </div>
                </div>
                -->
...
export default {
    ...
    methods:{
        ...
        _getText(num,flag){
            ...
            switch(num){
                case 0:
                    comtext = flag===3 ? '非常不专业' : '非常差';
                break;
                case 1:
                    comtext = flag===3 ? '非常不专业' : '非常差';
                break;
                case 2:
                    comtext = flag===3 ? '不专业' : '差';
                break;
                case 3:
                    comtext = "一般";
                break;
        ...
        _getComment() {
            ...
            //             case 200:
            //                 // 综合评分
            //                 that.comprehensive.comtext = that._getText(body.appraise,1);
            //                 that.comprehensive.goodcom = parseInt(body.appraise);
            //                 that.comprehensive.badcom = 5-parseInt(body.appraise);
            //                 // 服务态度评分
            //                 that.attitude.comtext = that._getText(body.attitude,2);
            //                 that.attitude.goodcom = parseInt(body.attitude);
            //                 that.attitude.badcom = 5-parseInt(body.attitude);
            //                 // 专业程度评分
            //                 that.major.comtext = that._getText(body.major,3);
            //                 that.major.goodcom = parseInt(body.major);
            //                 that.major.badcom = 5-parseInt(body.major);
            ...
            // debugger:end
            util.api({
                surl:root.COMMENT_API_PATH + 'findOne',
                type:'get',
                data:{
                     orderCode: that.hrefParma.otn
                },
                success:function(res) {
                    ...
                    switch(rpco) {
                        case 200:
                            // 综合评分
                            that.comprehensive.comtext = that._getText(body.totalScore,1);
                            that.comprehensive.goodcom = parseInt(body.totalScore);
                            that.comprehensive.badcom = 5-parseInt(body.totalScore);
                            // 服务态度评分
                            // that.attitude.comtext = that._getText(body.attitude,2);
                            // that.attitude.goodcom = parseInt(body.attitude);
                            // that.attitude.badcom = 5-parseInt(body.attitude);
