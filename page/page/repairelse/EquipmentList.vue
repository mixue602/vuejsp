<template>
    <ul class="el">
        <li v-for="(val,index) in body" class="elLi waves" :class="{'bdr-bottom': index!== body.length-1}">
            <label  @click="onAn(index,val.taf,val.brdid,val.brdnm)"><img :src="val.logo"  alt=""><i :class="{i:val.taf}"></i></label>
             <transition name="slide-fade">
                 <ul class="elUl clearfix" v-show="body[index].taf">
                    <li  class="float_l waves" @click="onshow(bodydata.models,index2,$event)" v-for="(val2,index2) in bodydata.models">{{ val2.mdnm }}</li>
                </ul>
            </transition>
        </li>
    </ul>
...
export default {
    ...
    methods:{
        onAn (index,parameter,brdid,brdnm) {
            ...
            that.brdnm = brdnm;
            util.api({
            surl:root.REPAIR_API_PATH+'queryModelsByBrand',
            data:{
        ...
        onshow (models,index) {
            ...
            var parmasObj = {
                //品类
                catename: this.hrefParma.eacnm,
                eacid: this.hrefParma.eacid,
                //品牌
                brdid: this.brdid,
                brdnm: this.brdnm,
                //型号
                mdld: models[index].mdld,
                attrnm: models[index].mdnm,
